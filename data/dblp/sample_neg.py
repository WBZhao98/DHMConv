...
for i in range(10):
    split='split'+str(i)
    edges = pd.read_csv(split + '/train.txt', sep='\t', header=None,dtype=int)
    (edges - 1).to_csv(split + '/train.txt', sep='\t', header=None, index=None)
    edges = pd.read_csv(split+'/test.txt', sep='\t', header=None,dtype=int)
...
# edges = pd.read_csv('train.txt', sep='\t', header=None,dtype=int)
#
# (edges-1).to_csv('citeseer.txt.txt',sep='\t',header=None,index=None)
