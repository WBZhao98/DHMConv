...
def test(model, users_to_test,nums, args, test_set,neg_test_dic,fts=[]):
...
if __name__ == '__main__':
    ...
    '''
    要改的东西：
    数据集：204行,模型名字：205行,读数据213行：readdata1：cora，citeseer。其他：readdata
    输入的G和模型：241，242行
    '''
    args.dataset = 'dblp'#Air,cora,citeseer,survey,amazon_computers
    modelname = 'HGNN'#HNHN,
    best = []
    ...
    for i in range(10):
        ...
        L, nums, edge_dic, train_dic, test_dic, neg_test_dic, n_train, n_test ,user_item_matrix,templ= loaddata_link.readdata(args)#cora,citeseer,
        ...
        # fts=torch.tensor(fts, dtype=torch.float32).to(args.device)
        ...
        #备选模型
        from models import HGNN1, HNHN, GraphSAGE, GCN, HGNNP, HyperGCN
        from dhg import Hypergraph,Graph
        G = Graph(nums,templ)
        HG = Hypergraph.from_graph_kHop(G,k=1)

        if modelname in ['HGNN','HNHN','HyperGCN','HGNNP']:
            G = HG #如果是超图模型（HGNN，HNHN，HyperGCN,HGNNP)则G=HG，其他图模型：G=G
        else:
            G = G
        model = HGNN1(nums,args.embed_size,128,64,drop_rate=args.node_dropout[0]).to(args.device)#修改模型名字即可
